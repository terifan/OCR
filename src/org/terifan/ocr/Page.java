...
public class Page implements Cloneable
{
	...
	public Page(BufferedImage aBufferedImage)
	{
		...
		mDebugGraphics = mDebugImage.createGraphics();

		mGraphics.drawImage(aBufferedImage, 0, 0, null);
		mDebugGraphics.drawImage(aBufferedImage, 0, 0, null);
	}
	...
	{
		BufferedImage tmp = new BufferedImage(mWidth, mHeight, BufferedImage.TYPE_INT_RGB);
	...
	public Page rotate(double aAngleDegrees)
	{
		mGraphics.dispose();
		mDebugGraphics.dispose();
		...
		if (aAngleDegrees == 90 || aAngleDegrees == 180 || aAngleDegrees == 270)
		{
			...
			rotateFixed(mDebugImage, (int)aAngleDegrees);
		}
		else if (aAngleDegrees != 0)
		{
			mImage = ImageRotator.rotate(mImage, aAngleDegrees, 1, 0xFFFFFFFF);
			mDebugImage = ImageRotator.rotate(mDebugImage, aAngleDegrees, 1, 0xFFFFFFFF);
		}
	...
	public void fillRect(int aOffsetX, int aOffsetY, int aWidth, int aHeight, byte c)
	{
		Color color = new Color(0xff&c,0xff&c,0xff&c);

		mGraphics.setColor(color);
		mGraphics.fillRect(aOffsetX, aOffsetY, aWidth, aHeight);

		mDebugGraphics.setColor(color);
	...
	public void eraseLines(double aMinInches, int aExtra)
	{
		...
		mDebugGraphics.setColor(Color.WHITE);
		...
		for (Rectangle r : list)
		{
			for (int y = -aExtra; y <= aExtra; y++)
			{
				...
				{
					mGraphics.drawLine(r.x + x, r.y + y, r.x + r.width + x, r.y + r.height + y);
